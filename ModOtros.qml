...
Item {
    ...
    property int modIngresarDatos: -1
    property var objIngresarDatos
   ...
   Button{
        ...
        onClicked: {
            modCompletado3=true
            modFichas.fichaCompletada=3;
        }

        Timer{
            id: timerControlUI
            running: tabBar.currentIndex===0&&app.area===3
            repeat: true
            interval: 500
            onTriggered: {

                    if(tiCaudal.text!==''&&tiBSW.text!==''&&tiDensity.text!==''&&tiPwf.text!==''&&tiCHP.text!==''&&tiTHP.text!==''&&tiTVD.text!==''&&tiIntake.text!==''&&cbBomba.currentIndex!==cbBomba.model.length-1&&cbBomba2.currentIndex!==cbBomba2.model.length-1){
                        botOK.enabled=false
                    }else{
                        botOK.enabled=true
                    }
